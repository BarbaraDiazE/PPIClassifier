...
if __name__ == "__main__":
    ...
    models_to_validate_m1 = models_to_validate(
        root=local_root["phase1"], id_validation="M1", algorithm="lrg", quartile="Q3"
    )
    ...
    models_to_validate_m2 = models_to_validate(
        root=local_root["phase1"], id_validation="M2", algorithm="lrg", quartile="Q3"
    )
    for j in range(len(models_to_validate_m2)):
        input_filename = f"dataset_{models_to_validate_m2[j][0]}.csv"
        model_name = models_to_validate_m2[j][1]
        A = Validation(
            root_data=local_root["data"], data_filename=input_filename, target="PPI", fraction=0.2
        )
        A.evaluate_model(root=local_root["phase1"], model_filename=model_name, cv=20)
