import os
import pandas as pd
import numpy as np
import joblib
from sklearn.preprocessing import label_binarize
from sklearn.model_selection import train_test_split
from sklearn.tree import DecisionTreeClassifier


from phase1.support_functions.dt.functions_DT import plot_roc, dt_report, plot_tree

# representative descriptors
custom_descriptors = [
    "HallKierAlpha",
    "VSA_EState7",
    "MinEStateIndex",
    "FractionCSP3",
    "NumAromaticHeterocycles",
    "NumHAcceptors",
    "NumRotatableBonds",
    "NumRotatableBonds",
    "NumHeteroatoms",
    "HeavyAtomMolWt",
]


class DT:
    def __init__(self, root_data, local_root, input_file, target, descriptors, fraction):
        data = pd.read_csv(os.path.join(root_data, input_file), index_col="Unnamed: 0")
        self.root_data = root_data
        ids = [
            "ipp_id",
            "chembl_id",
            "SMILES",
            "library",
            "PPI family",
            "PPI",
        ]
        print(
            "PPI types: ",
            data.PPI.unique(),
            "\n",
            "Libraries types: ",
            data.library.unique(),
            "\n",
            "Total compounds number: ",
            data.shape[0],
        )
        self.root_data = root_data
        self.local_root = local_root
        self.target = target
        self.descriptors = descriptors
        self.fraction = fraction
        numerical_data = data.drop(ids, axis=1)
        # use custom data
        self.numerical_data = numerical_data[custom_descriptors]
        self.data = data

    def train_model(self, criterion, max_depth, class_weight):
        """
        criterion: str {"gini", "entropy”}
        max_depth, int, default=None
        class_weight: str, {‘balanced’ or None}
        """
        y = np.array(self.data[self.target])
        y = label_binarize(y, classes=["No", "Yes"])
        # numerical_data = pd.DataFrame(
        #     StandardScaler().fit_transform(self.data[self.descriptors])
        # )
        x_train, x_test, y_train, y_test = train_test_split(
            self.numerical_data, y, test_size=self.fraction, random_state=1992
        )
        model = DecisionTreeClassifier(
            criterion=criterion,
            max_depth=max_depth,
            # min_samples_split=2,
            # min_samples_leaf=1,
            # min_weight_fraction_leaf=0.0,
            # max_features="auto",
            # max_leaf_nodes=None,
            random_state=2020,
            class_weight=class_weight,
            # ccp_alpha=0.0,
        )
        return model.fit(x_train, y_train), x_test, y_test

    @classmethod
    def get_attributes(cls, model):
        attributes = {
            "classes": model.n_classes_,
            # "tree": model.tree_,
            "feature_importance": model.feature_importances_,
        }
        print(model.feature_importances_)
        return attributes

    @classmethod
    def get_params(cls, class_weight, criterion, max_depth, fraction):
        parameters = {
            "Method": "DT",
            "class weight": class_weight,
            "criterion": criterion,
            "max_depth": max_depth,
            "fraction": fraction * 100,
        }
        return parameters

    @classmethod
    def get_predictions(cls, model, x_test, y_test):
        prediction_data = {
            "predictions": model.predict(x_test),
            "x_text": x_test,
            "y_test": y_test,
        }
        return prediction_data

    def report(self, criterion, max_depth, class_weight, output_reference):
        model, x_test, y_test = self.train_model(criterion, max_depth, class_weight)
        prediction_data = self.get_predictions(model, x_test, y_test)
        plot_tree(model, self.descriptors, self.local_root, output_reference)
        plot_roc(
            output_reference=output_reference,
            model=model,
            x=x_test,
            y=y_test,
            local_root=self.local_root,
        )
        dt_report(
            output_reference=output_reference,
            data=self.data,
            parameters=self.get_params(class_weight, criterion, max_depth, self.fraction),
            y_test=prediction_data["y_test"],
            predictions=prediction_data["predictions"],
            descriptors=self.descriptors,
            attributes=self.get_attributes(model),
            local_root=self.local_root,
        )
        trained_model_root = f"{self.local_root}/results/trained_results/trained_models"
        output_model_name = f"{output_reference}.pkl"
        joblib.dump(model, os.path.join(trained_model_root, output_model_name))
        return f"report {output_reference} is ready"
